	...
	namespace XML {
	   ...
	   having more than one XML manager per project! */
		namespace {
			bool load_all_tiles(const char* map_filepath, XMLNode* parent_node, TilePackage* tp) {
				...
				while (tile_element != nullptr) {
					...
					LOG_IF_AR(CRIT_LEVEL, tile_element->QueryUnsignedAttribute("x", &x) != XML_SUCCESS,
						"Map File '{0}' Contains a Tile With No X Attribute", map_filepath);
					LOG_IF_AR(CRIT_LEVEL, tile_element->QueryUnsignedAttribute("y", &y) != XML_SUCCESS,
						"Map File '{0}' Contains Tile With X '{1}' & No Y Attribute", map_filepath, x);
					LOG_IF_AR(CRIT_LEVEL, tile_element->QueryUnsignedAttribute("reg_num", &reg_num) != XML_SUCCESS,
						"Map File '{0}' Tile X '{1}' & Y '{2}' Has No reg_num Tag", map_filepath, x, y);
					LOG_IF_AR(CRIT_LEVEL, tile_element->QueryUnsignedAttribute("modifier_reg_num", &modifier_reg_num) !=
						XML_SUCCESS, "Map File '{0}' Tile X '{1}' & Y '{2}' Has No modifier_reg_num Tag", map_filepath, x, y);
					LOG_IF_AR(CRIT_LEVEL, (raw_text = tile_element->Attribute("script")) == nullptr,
		...
		TilePackage* load_map(const char* map_filepath) {
			...
			LOG_IF_AR(CRIT_LEVEL, size == nullptr, "Failed to Load Map '{0}', No Size Tag "
