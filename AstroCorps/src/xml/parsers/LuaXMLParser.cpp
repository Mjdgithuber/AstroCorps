...
bool LuaXMLParser::check_if_open(const std::string& caller) {
	if (!m_open) {
		// print warning
		LOG_WARN("Lua Parser \'{0}\' attempted to call \'{1}\' without an open \
			     file", m_name, caller);
...
bool LuaXMLParser::assert_current_element(const std::string& caller) {
	if (m_cached_element == nullptr) {
		// print warning
		LOG_WARN("Lua Parser \'{0}\' attempted to call \'{1}\' without an element \
			     currently loaded", m_name, caller);
...
bool LuaXMLParser::open_doc(const char* filename) {
	...
	if (m_open) {
		LOG_WARN("Lua Parser \'{0}\' attempted to open \'{1}\' before closing previous \
				 file \'{2}\'", m_name, filename, m_current_file);
...
/* Returns an int representation of the attribute with the given
   name, if applicable */
int LuaXMLParser::int_attribute(const std::string& attribute) {
	...
	// makes an int and attempts to load into it
	int i;
	XML::XMLError result = XML::load_int(m_cached_element, attribute.c_str(), i);

	// checks if error occured
	if (result != XML::XML_SUCCESS) {
		// log an error to the console
		LOG_ERROR("Lua Parser \'{0}\' tried to load int called \'{1}\' from element \'{2}\' from \
                  file \'{3}\'", m_name, attribute, m_cached_element->Name(), m_current_file);
		return -1; // don't crash just return -1
	}
	// return loaded int
	return i;
}

/* Returns a floating point representation of the attribute with the 
   given name, if applicable */
float LuaXMLParser::float_attribute(const std::string& attribute) {
	...
	// makes a float and attempts to load attribute into it
	float f;
	XML::XMLError result = XML::load_float(m_cached_element, attribute.c_str(), f);
	
	// check if error occured
	if (result != XML::XML_SUCCESS) {
		// log an error to the console
		LOG_ERROR("Lua Parser \'{0}\' tried to load float called \'{1}\' from element \'{2}\' from \
			      file \'{3}\'", m_name, attribute, m_cached_element->Name(), m_current_file);
		return -1.f; // don't crash instead return 1.f and let script handle it
	}
	// return loaded float
	return f;
}

/* Returns a string representation of the attribute with the given 
   name */
std::string LuaXMLParser::string_attribute(const std::string& attribute) {
	...
	// makes a new string and attempts to load the attribute into it
	std::string s;
	XML::XMLError result = XML::load_string(m_cached_element, attribute.c_str(), s);

	// check if error occured
	if (result != XML::XML_SUCCESS) {
		// logs error to console
		LOG_ERROR("Lua Parser \'{0}\' tried to load string called \'{1}\' from element \'{2}\' from \
			      file \'{3}\'", m_name, attribute, m_cached_element->Name(), m_current_file);
		return "ERROR"; // dont crash instead return "ERROR" to aid in debugging process
	}
