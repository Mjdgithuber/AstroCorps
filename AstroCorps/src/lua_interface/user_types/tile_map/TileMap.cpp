...
namespace Lua {
	...
	void TileMap::load_map(const char* map_file) {
		...
		const sf::Texture& tile_sheet = Engine::Register::get_tile_sheet();
	...
	void TileMap::register_with_lua(sol::state& state) {
		sol::usertype<TileMap> tile_map_type = state.new_usertype<TileMap>("TileMap",
